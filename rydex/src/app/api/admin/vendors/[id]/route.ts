...
import { generateAdminVendorSummary } from "@/lib/ai";
...
export async function GET(
  ...
  try {
    ...
    const documents = await VehicleDocument.findOne({
      ...
      .select(
        "aadhaarUrl licenseUrl rcUrl status rejectionReason aiReview"
      )
      .lean();
    ...
    const aiSummary = await generateAdminVendorSummary(vendorPayload);
