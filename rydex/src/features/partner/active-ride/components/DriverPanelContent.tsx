...
import { Car, Clock, IndianRupee, MessageCircle, Phone, User2 } from "lucide-react";
...
export function PanelContent({ booking, isActive, canChat, displayEta, chatOpen, onChatToggle }: PanelContentProps) {
  return (
        ...
        {chatOpen && canChat && (
          ...
          >
            <div className="rounded-2xl overflow-hidden border border-zinc-100 h-[460px]">
              <RideChat currentRole="driver" rideId={booking._id} userName={booking.user?.name} />
            </div>
          </motion.div>
        )}
      </AnimatePresence>
